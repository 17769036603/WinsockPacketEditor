...
class ProbeRuntime:
    ...
    def set_point_plan(
        ...
        self._point_plan = {
            ...
            "lastSequence": max(1, int(sequence)),
        }

    def get_point_plan(self, identity: dict) -> dict | None:
        identity_key = self._identity_key(identity)
        if identity_key is None or identity_key != self._point_identity:
            if self._point_plan is not None:
                self.clear_point_plan()
            return None
        if not isinstance(self._point_plan, dict) or parse_address(self._point_plan.get("lastSequence")) is None:
            self.clear_point_plan()
            return None
...
def probe_once(args: argparse.Namespace, runtime: ProbeRuntime | None = None) -> int:
    ...
    try:
        ...
        if isinstance(point_plan, dict):
            ...
            point_snapshot = None
            point_capture: dict = {}
            point_sequence = parse_address(point_plan.get("lastSequence"))
            if (
                ranges
                and point_sequence is not None
                and request_sequence > point_sequence
                and isinstance(point_cache, dict)
                and isinstance(point_hint, dict)
            ):
        ...
        if cached is not None:
            if ranges is None:
                ranges = filtered_ranges(session)
            cached_for_read = dict(cached)
            cached_for_read["ranges"] = (
            ...
            if warm_snapshot is not None:
                ...
                if not args.require_complete_refine_cards or complete_cards:
                    ...
                    emit_snapshot(
                        ...
                        warm_start=True,
                        read_path=read_path_override or "warm",
                    )
                    return 0
            read_path_override = "fallback"
