...
namespace Be.Windows.Forms
{
	...
	public class HexBox : Control
	{
		...
		void PaintHex(Graphics g, long startByte, long endByte)
		{
			...
			ByteStyleBrushCache styleBrushes = new ByteStyleBrushCache();
			...
			for (long i = startByte; i < intern_endByte + 1; i++)
			{
				...
				{
					ByteStyle style;
					if (_byteStyleProvider != null && _byteStyleProvider.TryGetStyle(i, out style))
						PaintHexStringStyled(g, b, style, gridPoint, styleBrushes);
					else
						PaintHexString(g, b, brush, gridPoint);
				}
			}
			styleBrushes.Dispose();
			brush.Dispose();
			selBrush.Dispose();
			selBrushBack.Dispose();
		}
		...
		void PaintHexStringStyled(Graphics g, byte b, ByteStyle style, Point gridPoint, ByteStyleBrushCache styleBrushes)
		{
			Brush foreBrush = styleBrushes.Get(style.ForeColor);
			Brush backBrush = styleBrushes.Get(style.BackColor);
			PointF bytePointF = GetBytePointF(gridPoint);
			bool isLastLineChar = (gridPoint.X + 1 == _iHexMaxHBytes);
			float width = isLastLineChar ? _charSize.Width * 2 : _charSize.Width * 3;
			g.FillRectangle(backBrush, bytePointF.X, bytePointF.Y, width, _charSize.Height);
			...
			for (long i = startByte; i < intern_endByte + 1; i++)
			{
				...
				else
				{
					...
					if (_byteStyleProvider != null && _byteStyleProvider.TryGetStyle(i, out style))
					{
						Brush foreBrush = styleBrushes.Get(style.ForeColor);
						Brush backBrush = styleBrushes.Get(style.BackColor);
						g.FillRectangle(backBrush, byteStringPointF.X, byteStringPointF.Y, _charSize.Width, _charSize.Height);
		...
		/// <summary>Gets or sets the optional per-byte style provider.</summary>
		[Browsable(false), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		public IByteStyleProvider ByteStyleProvider
		{
			get { return _byteStyleProvider; }
			set
			{
				if (_byteStyleProvider == value)
					return;
				_byteStyleProvider = value;
				Invalidate();
			}
		}
		IByteStyleProvider _byteStyleProvider;

		/// <summary>Returns the byte offset at a client point, or -1 outside the byte views.</summary>
		public long GetByteIndexAt(Point point)
		{
			if (_byteProvider == null)
				return -1;
			if (_recHex.Contains(point))
				return GetHexBytePositionInfo(point).Index;
			if (_recStringView.Contains(point))
				return GetStringBytePositionInfo(point).Index;
			return -1;
		}
		/// <summary>
